...
const PostFragment = ({
    ...
    const userLink = "/id" +postData.userId;
    ...
    let commentElements = postComments.map(comment => 
        <CommentContainer
            key={comment.id}
            id={comment.id}
            text={comment.text}
    ...
    const addCommentElement = React.useRef();
    ...
    const formatCommentText = formatText(commentUpdateText);

    // Действие добавления комментария на сервер
    const handleAddCommentBackend = () => {
        const date = new Date();
        const newCommentData = {
            text: formatCommentText,
            postId: postId,
            date: date,
            userId: postData.userId
        }

        addCommentBackend(newCommentData);
    ...
    // Действие обновления текста комментария
    const handleAddCommentText = () => {
        const commentText = addCommentElement.current.value;
        addCommentText(commentText);
    }

    // Действие лайка поста
    const handleLikeButton = () => {
        likeBackend(postData.id)
    }

    // Действие удаления лайка с поста
    const handleDeleteLikeButton = () => {
        deleteLikeBackend(postData.id)
    }

    // Действие добавления страницы
    const handleAddPage = async () => {
        addPageBackend();
    }

    // Действие открытия эмодзи окна
    ...
    const handleEmojiSelect = (emoji) => {
        const textarea = addCommentElement.current;
        ...
        addCommentText(updatedText); // обновляем значение postUpdateText
    }

    // Действие закрытия эмодзи окна
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter' && !e.shiftKey && window.innerWidth > 768) {
            e.preventDefault();
            if (commentUpdateText.trim() !=='' || commentUpdateText.trim().length !== 0) {
                handleAddCommentBackend();
            }
        }
    };
    ...
    video.src = postData.file;

    video.onloadedmetadata = () => {
        setIsVertical(video.videoHeight > video.videoWidth);
    }; 

    const linkedText = Autolinker.link(postData.text, {
        hashtag: 'twitter',
        truncate: {
            length: 20,
            location: 'end'
                    ...
                    <div className="post-container">
                        <div className="post-header">
                            <div className="post-author">
                                ...
                                >
                                    <div className={postData.avatarType === 'nft' ? "post-author-avatar-image post-author-avatar-nft" : "post-author-avatar-image"} style={{ display: !showContent && 'none' }}>
                                        <img src={postData.avatar ? postData.avatar : Avatar} />
                                    </div>
                                    {!showContent && (
                                ...
                                <div className="post-author-info">
                                    <div className="post-author-header">
                                        ...
                                        >
                                            <span>{!postData.name ? userFriendlyAddress(postData.wallet) : postData.name}</span>
                                        </NavLink>
                                        {postData.verify ? (
                                    ...
                                    <div className="post-author-date">
                                        <span><ReactTimeAgo date={Date.parse(postData.date)} locale="ru-RU"/></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        {postData.text && (
                            <div className="post-text">
                                <Emoji options={{className: 'emoji'}} className="text">
                        ...
                        {postData.file ? (
                            <>
                                ...
                                <div className={showContent ? "post-file" : "post-file hidden"}>
                                    {postData.fileType === 'image' && (
                                        <div className={isVertical ? "post-image post-image-vertical" : "post-image"}>
                                            <img src={postData.file} />
                                            {isVertical ? (
                                                <div className="post-image-blur" style={{backgroundImage: `url(${postData.file})`}}></div>
                                            ) : ''}
                                        </div>
                                    )}
                                    ...
                                    {postData.fileType === 'video' && (
                                        <div className={isVertical ? "post-video post-video-vertical" : "post-video"}>
                                            <div className="post-video-player">
                                                <video autoPlay={true} controls={false} playsInline muted={muteSound} loop>
                                                    <source src={postData.file}type="video/mp4" />
                                                </video>
                                                {muteSound ? (
                            ...
                            <div className="post-action-items">
                                {authUserId ? (
                                    <div className="post-action-item">
                                        {postData.like ? (
                                            <div className="post-action-button active" onClick={handleDeleteLikeButton}>
                                            ...
                                            <div className="post-action-button" onClick={handleLikeButton}>
                                                <div className="post-action-icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 733.33 600">
                                                        <path d="M533.33,0c-70,0-131.67,35-166.67,90C331.67,35,270,0,200,0,90,0,0,90,0,200c0,198.33,366.67,400,366.67,400,0,0,366.67-200,366.67-400C733.33,90,643.33,0,533.33,0h0Z"/>
                                                    </svg>
                    ...
                    <div className="form-container">
                        <div className="form-items">
                            ...
                            <div className="form-button">
                                ...
                                <button  className="button" disabled={!commentUpdateText.trim() || commentUpdateText.trim().length === 0} onClick={handleAddCommentBackend}>
                                    <div className="form-button-icon">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 800">
