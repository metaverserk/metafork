...
import {getFollowing, getFollowingPage } from '../../../api/profile';
...
const Users = ({
    ...
    useEffect(() => {
        ...
        document.title = 'Подписки пользователя';
        fetchData();
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await getFollowing(requestData);
    ...
    const addPageBackend = async () => {
        try {
            const requestData = { userId };
            const response = await getFollowingPage(currentPage, requestData);
